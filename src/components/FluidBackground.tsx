...
void main() {
  ...
  // Cycle hue over time: Blue(0.55) -> Cyan -> Green -> Yellow -> Red(0.0/1.0) -> back
  // Full cycle every 60 seconds — complete 360° hue rotation
  float hueCycle = mod(u_time * 0.01667, 1.0);
  float hue = 0.55 + hueCycle * 1.0; // full circle: blue → purple → red → yellow → green → cyan → blue
  if (hue > 1.0) hue -= 1.0;

  // Lighter/brighter smoke intensities map to more saturated, brighter colors
  // Darker smoke areas stay dark/deep
  float brightness = 0.03 + pattern * pattern * 0.55;
  float saturation = 0.4 + pattern * 0.5;
